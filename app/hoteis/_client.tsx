...
export default function HotelsIndexClient({ items }: { items: Hotel[] }) {
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 md:px-6 py-8">
      <header className="mb-6">
        ...
        <h1 className="mt-1 text-2xl md:text-3xl font-extrabold text-[#E5DCC9]">Hotéis & Pousadas</h1>
      ...
      <AnimatePresence initial={false} mode="popLayout">
        ...
        >
            ...
            >
              <Link href={h.slug ? `/hotel/${h.slug}` : "#"} className="block">
                ...
                <div className="p-4">
                  ...
                  {h.description && (
                    <p className="text-white/70 text-sm mt-1 line-clamp-2">{h.description}</p>
                  )}
                </div>
              </Link>
            </article>
          ...
          {!filtered.length && (
            <div className="col-span-full text-white/70">Nenhum hotel encontrado.</div>
          )}
        </motion.div>
      </AnimatePresence>
    </section>
